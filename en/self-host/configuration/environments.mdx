...
- DB_TYPE: Database type.
  ...
  - `postgresql`(default)
  - `mysql` (MySQL-compatible databases like OceanBase can also use this value)
  - `seekdb` (lightweight version of OceanBase, MySQL-compatible)
...
- OCEANBASE_VECTOR_HOST
  ...
  The hostname or IP address of OceanBase vector database. Also used for seekdb when `VECTOR_STORE=seekdb`.

- OCEANBASE_VECTOR_PORT

  The port of OceanBase vector database. Also used for seekdb when `VECTOR_STORE=seekdb`.

- OCEANBASE_VECTOR_USER

  The username of OceanBase vector database. Also used for seekdb when `VECTOR_STORE=seekdb`.

- OCEANBASE_VECTOR_PASSWORD

  The password of OceanBase vector database user. Also used for seekdb when `VECTOR_STORE=seekdb`.

- OCEANBASE_VECTOR_DATABASE

  The database name of OceanBase vector database. Also used for seekdb when `VECTOR_STORE=seekdb`.
...
- SEEKDB_MEMORY_LIMIT
  ...
  The memory limit of seekdb, only available for Docker deployment. seekdb is the lightweight version of OceanBase and uses the same connection parameters as OceanBase (OCEANBASE_VECTOR_* variables) when configured as a vector store.
